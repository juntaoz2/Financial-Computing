...
double PathDepOption::PriceByMC(const BSModel& Model, long N,double epsilon){
    ...
    gamma = exp(-Model.GetR()*T)*(Heps-2*H+Hgam)/pow((Model.GetS0()*epsilon),2.0);
    return Price;
}

double PathDepOption::PriceByVarRedMC(const BSModel& Model, long N, PathDepOption& CVOption, double epsilon){
    ...
    delta1 = VarRedOpt.delta+CVOption.GetDelta();
    return Price;
}
